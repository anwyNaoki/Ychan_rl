...
class Brain:
    def __init__(self):   # globalなparameter_serverをメンバ変数として持つ
        with tf.name_scope("brain"):
            ...
            self.d_model.compile(optimizer=Adam(0.0002, 0.5),loss='binary_crossentropy', 
            metrics=['accuracy'])
            self.prob_old = 1
    ...
    def _build_model(self):     # Kerasでネットワークの形を定義します
        ...
        models = Model(inputs=[l_input], outputs=[out_actions, out_value])
        models._make_predict_function()  # have to initialize before threading
        #plot_model(models, to_file='PPO.png', show_shapes=True)  # Qネットワークの可視化
        return models

    def _build_d_model(self):
        l_input = Input(batch_shape=(None, NUM_STATES + 1)) #状態とアクションからG-E判断をする
        l_dense = Dense(32, activation='relu')(l_input)
        l_dense = Dense(32, activation='relu')(l_input)
        out_value = Dense(1, activation='sigmoid')(l_dense)
    ...
    def update_d(self):
        ...
        a = [[_a] for _a in expert_actions]
        s_a = np.concatenate([expert_observations, a], 1)
        one = np.ones(len(a))
        self.d_model.train_on_batch(s_a, one)

        s, a, r, s_, s_mask = self.train_queue
        a = [[np.argmax(_a)] for _a in a]
        s_a = np.concatenate([s, a],1)
        one = np.zeros(len(s_a))
        self.d_model.train_on_batch(s_a, one)

        self.d_rewards = self.d_model.predict(s_a)
...
class Environment:
    ...
    def run(self):
        ...
        while True:
            ...
            s = s_
            R += r
            if done:  # 終了時がTmaxごとに、parameterServerの重みを更新
                if not (isLearned) and self.thread_type is 'learning':
                    self.agent.brain.update_d()
...
expert_observations = np.genfromtxt('observations.csv')
expert_actions = np.genfromtxt('actions.csv', dtype=np.int32)
thread_name = "local_thread" + str(0)
brain = Brain()
environment = Environment(name=thread_name, thread_type="learning", brain=brain)
while True:
    try:
        environment.run()
    except KeyboardInterrupt:
        break
environment = Environment(name=thread_name, thread_type="test", brain=brain)
environment.run()
